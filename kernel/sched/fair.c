...
{
	__update_entity_runnable_avg(rq_clock_task(rq), cpu_of(rq), &rq->avg, runnable,
				     runnable);
...
static inline void __update_task_entity_contrib(struct sched_entity *se)
{
	...
	if (!hmp_cpu_is_fastest(cpu_of(se->cfs_rq->rq)) &&
		se->avg.load_avg_contrib > (hmp_power_migration ? hmp_up_power_threshold : hmp_up_threshold))
...
static unsigned int hmp_down_migration(int cpu, struct sched_entity *se)
{
	...
	if (cpumask_intersects(&hmp_slower_domain(cpu)->cpus,
		...
		if (hmp_power_migration && is_efficient_down(se->avg.load_avg_contrib))
			return 1;
 
		if (!hmp_power_migration && se->avg.load_avg_contrib < down_threshold)
			return 1;
	}
	return 0;
}
